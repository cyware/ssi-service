...
This command creates a webhook inside of the SSI-Service. It tells the SSI-Service to make a POST request to the specified URL (http://my-service-that-recieves-webhooks.com/webhook) after a DID is created (i.e. after the [Create DID Document](https://developer.cyware.github.io/docs/apis/ssi-service#tag/WebhookAPI) endpoint is called).
...
Use the SSI Service to set up a Presentation Definition and corresponding Presentation Request to accept a KYC credential. We have the prerequisites for this in our [credential_manifest integration test](https://github.com/cyware/ssi-service/blob/main/integration/credential_manifest_integration_test.go)
